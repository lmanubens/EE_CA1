...
% outs={};
% for i=1:length(tree)
%     nneuron{i}.pp{1}.IClamp = struct('node',1,'times',[50 250],'amp',[.35 0]);     % add a current clamp electrode to the first node and define stimulation times [ms] and amplitudes [nA]
%     nneuron{i}.record{1}.IClamp = struct('node',1,'record','i');                   % record the current of the IClamp just for visualization
% 
%     out = t2n(nneuron{i},tree{i},'-w-q');                                      % execute t2n and receive output
%     outs{i}=out;
% end
% % 
...
% for i = 1:length(tree)
%     plot_syninp(tree{i}, outs{i}, 1, 45/neuron{i}.params.dt);
% end
% 
% figure; 
...
%% Several simulations with different injected current amplitudes (f-I relationship)
...
% %
% amp = 0:.2:1;                  % define a series of amplitudes for the electrode (here: 6 steps from 0 to 0.6 nA)
% for i = 1:length(tree)
...
% 
%     nneuron{1} = neuron{i};                                                               % use neuron structure defined above as basis for the first simulation instance
%     for n = 1:numel(amp)
...
%% Several voltage clamp steps (I-V relationship)
...
%% Small network with Poissonian input
