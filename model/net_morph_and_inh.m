...
%     trees{2} = struct('artificial','VecStim');                             % add a VecStim as an artificial cell. This class can have a vector played to it defining its activity
    ...
    trees = t2n_writeTrees(trees,[],fullfile(pwd,'inh_net.mtr'));                                    % tree morphologies are rewritten because this NetStim might be not written yet
...
csvwrite('numspikes_both.csv',horzcat(numspikes{:})')
...
n = length(avact);          % number of samples
...
fid = fopen('pspectrum_both.csv','w'); 
fprintf(fid,'%s\n',textHeader)
fclose(fid)
%write data to end of file
dlmwrite('pspectrum_both.csv',[f',powerpltwt',powerplt'],'-append');
